...
// Native macOS aesthetic — clean, spacious, layered
// ============================================================
...
const activeTabStyle: React.CSSProperties = {
  ...
  color: "#F5F5F7",
  boxShadow: "0 1px 3px rgba(0, 0, 0, 0.25), 0 0.5px 0 rgba(255, 255, 255, 0.04) inset",
};
      ...
      <div style={scrollArea}>
        <div style={cardStyle}>
          ...
          <div style={brandingStyle}>
            ...
            <p style={taglineStyle}>Voice dictation that thinks before it types</p>
          </div>

          {/* Sign In / Create Account toggle */}
          <div style={tabBarStyle}>
            <button style={getTabStyle("login")} onClick={() => handleTabSwitch("login")} onMouseEnter={() => setHoveredTab("login")} onMouseLeave={() => setHoveredTab(null)}>
              Sign In
            </button>
            <button style={getTabStyle("register")} onClick={() => handleTabSwitch("register")} onMouseEnter={() => setHoveredTab("register")} onMouseLeave={() => setHoveredTab(null)}>
              Create Account
            </button>
          </div>
          ...
          <div style={{ width: "100%" }}>
            {activeTab === "login" ? (
              <LoginForm onSubmit={handleLogin} onForgotPassword={handleForgotPassword} isLoading={isLoading} error={error} />
            ) : (
          ...
          <div style={oauthGroupStyle}>
            <Button type="button" variant="secondary" size="md" fullWidth icon={appleIconSvg} onClick={() => handleOAuth("apple")} disabled={isLoading}>
              Sign in with Apple
            </Button>
            <Button type="button" variant="secondary" size="md" fullWidth icon={googleIconSvg} onClick={() => handleOAuth("google")} disabled={isLoading}>
              Sign in with Google
            </Button>
          </div>
        </div>
